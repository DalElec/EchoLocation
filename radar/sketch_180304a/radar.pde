    ...
    void drawObject() {
      ...
      if(iAngle==0){
        ...
        rect(objects[x]*cos(radians(x)),-objects[x]*sin(radians(x)), 2, 2);
      }
    }
      popMatrix();
    }
    void drawLine() {
      ...
      if(iAngle>0){
        line(0,0,(height-height*0.12)*cos(radians(i)),-(height-height*0.12)*sin(radians(i))); // draws the line according to the angle
